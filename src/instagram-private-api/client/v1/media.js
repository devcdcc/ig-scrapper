...
Media.likersRaw = function(session, mediaId, maxId = false) {
    let request = new Request(session)
        .setMethod('GET')
    if(maxId)
        request = request.setResource('mediaLikes', {mediaId: mediaId, max_id: maxId})
    else
        request = request.setResource('mediaLikes', {mediaId: mediaId})
    return request
        .send()
        .then(function(data) {
            return data;
